...
def build_base_xml(batch=1000, type=None, startRow=None, endRow=None, startTime=None, endTime=None, maxVersions=1):
    if type is None:
...
def build_row_filter(row_value, operation, comparator='binary', batch=1000, type=None, startRow=None,
                    endRow=None, startTime=None, endTime=None, maxVersions=1):
    #{"op": "LESS", "type": "RowFilter", "comparator": {"value": "dGVzdFJvd09uZS0y", "type": "BinaryComparator"}}
    xml = build_base_xml(
        batch,
    ...
    if comparator == 'binary':
        filter.text = '{"op":"%s","type":"RowFilter", "comparator": {"value": "%s", "type": "BinaryComparator"} }' \
                      % (operation, b64_encoder(row_value))
    else:
        filter.text = '{"op":"%s","type":"RowFilter", "comparator": {"value": "%s", "type": "RegexStringComparator"} }' \
                      % (operation, row_value)

    return et.tostring(xml).decode('utf-8')


def build_value_filter(value, operation, comparator='binary', batch=1000, type=None, startRow=None,
                    endRow=None, startTime=None, endTime=None, maxVersions=1):
    # {"op":"EQUAL","type":"ValueFilter","comparator":{"value":"dGVzdFZhbHVlT25l","type":"BinaryComparator"}}
    ...
    filter = et.SubElement(xml, "filter")
    if comparator == 'binary':
        filter.text = '{"op":"%s","type":"ValueFilter", "comparator": {"value": "%s", "type": "BinaryComparator"} }' \
                      % (operation, b64_encoder(value))
    else:
        filter.text = '{"op":"%s","type":"ValueFilter", "comparator": {"value": "%s", "type": "RegexStringComparator"} }' \
                     % (operation, value)

    return et.tostring(xml).decode('utf-8')


def build_single_column_value_filter(family, qualifier, value, operation, comparator='binary', batch=1000, type=None,
                                     startRow=None, endRow=None, startTime=None, endTime=None, maxVersions=1):
    """
{
  ...
  "comparator": {
    ...
    if comparator == 'binary':
        filter.text = '{"op":"%s","type":"SingleColumnValueFilter","family":"%s", "qualifier":"%s", "comparator": {"value": "%s", "type": "BinaryComparator"}}' \
                      % (operation, b64_encoder(family), b64_encoder(qualifier), b64_encoder(value))

    else:
        filter.text = '{"op":"%s","type":"SingleColumnValueFilter","family":"%s", "qualifier":"%s", "comparator": {"value": "%s", "type": "RegexStringComparator"}}' \

...
class TestGet(TestCase):
    ...
    def test_get(self):
        ...
        result_2_versions = self.get.get('test_get', 'row-1', 'cf:name', number_of_versions=3)
        result_timestamp = self.get.get('test_get', 'row-1', 'cf:name', timestamp=timestamp)
        name = Bytes.to_string(result_2_versions['row'][0]['cell'][0]['$'])
        self.assertEqual(name, 'JohnNew')
        self.assertEqual(Bytes.to_string(result_timestamp['row'][0]['cell'][0]['$']), "John")
